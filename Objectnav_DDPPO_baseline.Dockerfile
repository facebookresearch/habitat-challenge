...
RUN /bin/bash -c ". activate habitat; git clone http://github.com/facebookresearch/habitat-api.git habitat-api2; cd habitat-api2; git fetch; git checkout master; cd .."
#RUN /bin/bash -c ". activate habitat; git clone http://github.com/facebookresearch/habitat-api.git habitat-api2; cd habitat-api2; git fetch; git checkout objectnav_ddppo_agent; cd .."
RUN /bin/bash -c ". activate habitat; cd habitat-api2; git status; cd .."
#RUN /bin/bash -c ". activate habitat; rm -rf habitat-api/habitat/tasks"
...
ADD ddppo_objectnav_agent.py agent.py
ADD submission.sh submission.sh
ADD configs/challenge_objectnav2020.local.rgbd.yaml /challenge_objectnav2020.local.rgbd.yaml
ADD configs/ configs/
#ADD habitat-challenge-data/objectgoal_mp3d/val_mini/val_mini.json.gz habitat-challenge-data/objectgoal_mp3d/val_mini/val_mini.json.gz
